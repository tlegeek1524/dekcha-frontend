...
export default function Login() {
  ...
  const initializeLiff = useCallback(async () => {
    try {
      // 1. ตรวจสอบและลบ query parameters ที่ LIFF อาจจะเพิ่มเข้ามา
      ...
      if (liff.isLoggedIn()) {
        ...
        if (idToken) {
          // หากล็อกอินสำเร็จและมี ID Token
          // ทำการส่งข้อมูลไปที่ Backend (หรือประมวลผล Frontend) ก่อน Redirect
          // ตัวอย่าง: การส่ง ID Token ไปที่ Backend
          console.log("Logged in! User ID Token:", idToken);
          message.success({ content: 'เข้าสู่ระบบสำเร็จ! กำลังเปลี่ยนหน้า...', key: LOGIN_MESSAGE_KEY });

          // หลังจากที่ประมวลผลข้อมูลเสร็จแล้ว
          // ทำการ Redirect ไปยังหน้า '/login/userlogin'
  ...
  useEffect(() => {
    // โหลด font และเริ่มต้น LIFF เมื่อ component โหลดครั้งแรก
  ...
  // ส่วน UI ที่แสดงสถานะ Loading และหน้า Login
